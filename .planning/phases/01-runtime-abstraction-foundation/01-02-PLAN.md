...
<task type="auto">
  ...
  <files>packages/core/src/runner/job-executor.ts</files>
  <action>
...
3. **Update execute() method:**
   Replace the SDK query call (around line 210-231) with:
   ```typescript
...
4. **Update executeJob convenience function:**
...
5. **Keep SDKQueryFunction type** for backwards compatibility with tests, but add deprecation comment:
   ```typescript
   /**
...
6. **Remove toSDKOptions import** - no longer needed in JobExecutor since SDKRuntime handles this.
   The import `import { toSDKOptions } from "./sdk-adapter.js";` can be removed if only used for SDK options.
   Check: toSDKOptions is called on line 196 - this moves to SDKRuntime, so remove the call from JobExecutor.

CRITICAL: The for-await loop processing messages (lines 233-327) stays UNCHANGED. Only the source of messages changes from this.sdkQuery() to this.runtime.execute().

IMPORTANT: Do NOT remove the AbortController parameter - ensure it's in the RunnerOptionsWithCallbacks type and passed through.
  </action>
  <verify>
...
Tests still import correctly: `cd packages/core && pnpm exec tsc --noEmit src/runner/__tests__/job-executor.test.ts`
  ...
  <done>JobExecutor constructor accepts RuntimeInterface, execute() calls this.runtime.execute()</done>
</task>
