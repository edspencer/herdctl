...
must_haves:
  truths:
    ...
    - "RuntimeFactory.create() receives stateDir from all call sites"
...
<task type="auto">
  ...
  <files>packages/core/src/scheduler/schedule-runner.ts, packages/core/src/fleet-manager/job-control.ts, packages/core/src/fleet-manager/schedule-executor.ts</files>
  <action>
Update all three RuntimeFactory.create() call sites to pass stateDir option. All sites already have stateDir in scope.

**1. schedule-runner.ts line 354:**
```typescript
// Before:
...
```
Note: `stateDir` is already in scope from destructured options at line 251.

**2. job-control.ts line 133:**
```typescript
// Before:
const runtime = RuntimeFactory.create(agent);
...
const runtime = RuntimeFactory.create(agent, { stateDir });
```
Note: `stateDir` is already in scope from `this.ctx.getStateDir()` at line 75.

**3. schedule-executor.ts line 92:**
```typescript
// Before:
const runtime = RuntimeFactory.create(agent);
...
const runtime = RuntimeFactory.create(agent, { stateDir });
```
Note: `stateDir` is already in scope from `this.ctx.getStateDir()` at line 70.
  </action>
  <verify>
Run `pnpm build` from packages/core - all call sites should compile.
Verify RuntimeFactory calls include stateDir option at lines 354, 133, and 92 respectively.
  </verify>
  <done>
All three RuntimeFactory.create() call sites pass stateDir option for Docker session management.
  </done>
</task>

</tasks>

<!-- NOTE: Task 3 removed - ContainerRunner already creates docker-sessions directory
     at container-runner.ts lines 444-446 (Plan 03-02 Task 2). No StateManager exists;
     state module uses functional utilities. -->
...
5. Docker configuration flows correctly: agent.docker -> resolveDockerConfig -> ContainerRunner
</verification>
