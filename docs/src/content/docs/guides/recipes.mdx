...
async function runOnce(agentName: string, prompt?: string) {
  ...
  try {
    ...
    // trigger() resolves when the job has finished, so no event
    // subscription is needed — inspect the result directly
    const result = await manager.trigger(agentName, undefined, { prompt });

    if (!result.success) {
      throw result.error ?? new Error(`Job ${result.jobId} failed`);
    }

    console.log(`Job ${result.jobId} completed`);

    // Optionally fetch the agent's final response
    const output = await manager.getJobFinalOutput(result.jobId);
    console.log(output);
  } finally {
    await manager.stop();
  }
...
async function main() {
  ...
  try {
    ...
    // trigger() resolves when the job has finished. Stream output to the
    // CI log as it happens via the onMessage callback, and enforce a
    // timeout by cancelling the job once its ID is known.
    let timeoutHandle: NodeJS.Timeout | undefined;

    const result = await manager.trigger(agentName, undefined, {
      prompt,
      onJobCreated: (jobId) => {
        console.log(`Job started: ${jobId}`);
        timeoutHandle = setTimeout(() => {
          console.error(`\n✗ Job timed out after ${timeout / 1000}s, cancelling`);
          manager.cancelJob(jobId).catch((error) => {
            console.error("Failed to cancel job:", error);
          });
        }, timeout);
      },
      onMessage: (message) => {
        // Print the agent's text output as it streams
    ...
    if (result.success) {
      ...
      console.error(`\n✗ Job ${result.jobId} failed: ${result.error?.message ?? "unknown error"}`);
    }
...
<Aside type="note">
Don't rely on `job:created` / `job:output` events to follow a manual `trigger()` run — `job:output` is never emitted for manual triggers, and `job:created` fires only after the job has already finished ([#328](https://github.com/edspencer/herdctl/issues/328)). Use the `onMessage` and `onJobCreated` callbacks as shown above, or `getJobFinalOutput()` after `trigger()` resolves. The job events work as documented for scheduled runs.
</Aside>
