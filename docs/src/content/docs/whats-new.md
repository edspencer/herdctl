...
Fixed path traversal checks failing on Windows systems. The `buildSafeFilePath` utility was hardcoded to use "/" as the path separator when validating that resolved paths stay within base directories. On Windows, `path.resolve` returns paths with "\", causing the `startsWith` check to always fail and throw false positive `PathTraversalError` on every state file operation. The function now uses `path.sep` for cross-platform compatibility and handles edge cases like root directory base paths that already end with a separator. [#210](https://github.com/edspencer/herdctl/pull/210)
