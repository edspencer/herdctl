...
1. **Get an OpenAI API key**
   ...
   Sign up at [platform.openai.com](https://platform.openai.com/) and create an API key with access to the Whisper API.
...
<Aside type="tip">
  The `language` parameter is optional but recommended for better transcription accuracy. Use ISO 639-1 language codes like `"en"` for English, `"es"` for Spanish, etc.
</Aside>
...
#### Example Attachments Configuration
...
| Category | MIME Pattern | Examples | Processing |
|----------|-------------|----------|------------|
...
<Aside type="caution">
  Set appropriate `max_file_size_mb` limits based on your OpenAI plan. Large files consume more tokens and may cause rate limiting or high costs.
</Aside>
...
<Aside type="tip">
  In production or containerized environments, use an explicit skills list to avoid filesystem dependency and ensure consistent behavior.
</Aside>
