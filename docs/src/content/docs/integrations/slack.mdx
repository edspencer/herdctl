...
<Aside type="note">
  Unlike Discord, Slack does not have guild (server) hierarchy, presence settings, or output formatting options. Slack DM configuration uses the same `dm` schema as Discord.
</Aside>
