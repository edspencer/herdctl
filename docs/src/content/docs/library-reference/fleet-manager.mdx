...
`copy` is the default so originals are never mutated unless explicitly requested. Copies preserve the source mtime because mtime drives both list ordering and every metadata cache key. Existing destination files are never overwritten.
