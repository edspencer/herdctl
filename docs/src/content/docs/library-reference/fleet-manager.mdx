...
| `options.claudeHomePath` | `string` | No | `~/.claude` | The Claude home directory holding `projects/<encoded-cwd>/<session-id>.jsonl` transcripts. Threaded into session discovery *and* every runtime, and exported to the Claude Code process itself as `CLAUDE_CONFIG_DIR`, so listing, reading and writing all resolve against the same home. Must match whatever home the embedding app resolved — a mismatch makes sessions list but open empty. |
...
interface DiscoveredSession {
  ...
  autoName: string | undefined;     // Auto-generated name (session title, else first-message preview)
...
Sessions with `origin: 'adopted'` are pre-existing transcripts claimed via [`adoptSession`](#adoptsessionname-sessionid-opts) or [`adoptSessionsFrom`](#adoptsessionsfromname-opts). They behave like any other session here.
...
### `listAdoptableSessions(name, fromWorkingDir?)`

Lists the pre-existing Claude Code sessions an agent could adopt.

```typescript
manager.listAdoptableSessions(
  name: string,
...
|-----------|------|----------|-------------|
| `name` | `string` | **Yes** | Agent qualified name or local name |
| `fromWorkingDir` | `string` | No | Directory whose transcript folder to scan. Defaults to the agent's own working directory |
...
interface AdoptableSession {
  sessionId: string;
  sourceCwd: string;      // Directory the transcript was found under
  mtime: string;          // ISO 8601, drives sort order
...
Newest first. Empty when the agent has no working directory and no explicit directory was given.

#### Description

Returns native, non-sidechain transcripts that are not already adopted and are not attributed to a real run — i.e. exactly the sessions that are invisible under the agent today. Intended to back an "import my existing chats" picker.

Resolved against the fleet's configured Claude home (see [`getClaudeHomePath()`](#getclaudehomepath)). Native CLI transcripts only: a Docker-wrapped agent's sessions live in `<stateDir>/docker-sessions/`, so this returns nothing for one unless a CLI-side directory is named explicitly. Use [`adoptSession`](#adoptsessionname-sessionid-opts) to claim a docker session by ID.

There is no message count, deliberately — obtaining one means streaming the whole transcript, which is the per-session cost the listing caches exist to avoid.
...
### `adoptSession(name, sessionId, opts?)`

Adopts a single pre-existing Claude Code session into an agent.
...
|-----------|------|----------|-------------|
| `name` | `string` | **Yes** | Agent qualified name or local name |
| `sessionId` | `string` | **Yes** | The Claude Code session ID to adopt |
| `opts.sourceCwd` | `string` | No | Working directory the transcript was recorded under. Defaults to the agent's own working directory |
...
Records an adoption claim in `<stateDir>/adopted-sessions/` — a real, dedicated attribution source, deliberately not a forged job record — so the session becomes discoverable under the agent with `origin: "adopted"` and resumable like any other.

**No file is moved.** Use this when the transcript is already in the agent's own transcript folder (the "point an agent at a directory I've already used" case); use [`adoptSessionsFrom`](#adoptsessionsfromname-opts) when it lives under a different working directory.
...
| `InvalidStateError` | Fleet manager not initialized |
| `AgentNotFoundError` | Agent doesn't exist |
| `PathTraversalError` | `sessionId` is not a safe identifier |

#### Example

```typescript
const record = await manager.adoptSession('my-agent', '0f3c8b2a-...');
console.log(`Adopted at ${record.adoptedAt}`);

const sessions = await manager.getAgentSessions('my-agent');
sessions.find((s) => s.sessionId === record.sessionId)?.origin; // 'adopted'
```

---

### `adoptSessionsFrom(name, opts?)`

Adopts every adoptable session found in a working directory.

```typescript
manager.adoptSessionsFrom(
  name: string,
  opts?: {
    fromWorkingDir?: string;
    mode?: 'copy' | 'move' | 'link';
...
|-----------|------|----------|---------|-------------|
| `name` | `string` | **Yes** | — | Agent qualified name or local name |
| `opts.fromWorkingDir` | `string` | No | Agent's working directory | Source directory to adopt from |
| `opts.mode` | `AdoptionPlacementMode` | No | `'copy'` | How to place the transcript into the agent's folder |
| `opts.dryRun` | `boolean` | No | `false` | Perform no writes at all; report what would have happened |

#### Returns

```typescript
interface AdoptSessionsResult {
  adopted: string[];                    // Session IDs adopted
  skipped: AdoptSkippedSession[];       // Everything else, with a reason
}

interface AdoptSkippedSession {
  sessionId: string;
  reason:
    | 'sidechain'
    | 'already-adopted'
...
Discovery looks for an agent's transcripts under `<claudeHome>/projects/<encoded agent cwd>/`, so a transcript recorded under a different directory is not where discovery will look. For each candidate this places the transcript into the agent's own folder, then records attribution with the originating directory as `sourceCwd`. When the source directory already resolves to the agent's own folder, nothing is moved and only attribution is recorded.

Placement modes:

| Mode | Effect |
|------|--------|
| `copy` (default) | Duplicate the file, preserving the source mtime. The user's original `~/.claude` transcript is untouched |
...
`copy` is the default so originals are never mutated unless explicitly requested. Copies preserve the source mtime because mtime drives both list ordering and every metadata cache key. Existing destination files are never overwritten: every mode creates the destination *exclusively* (`COPYFILE_EXCL` for a copy, `link`/`symlink` for the other two), so a transcript already at the destination is skipped as `destination-exists` even if it arrives between the check and the placement.

One bad transcript never aborts the batch — it lands in `skipped` with a reason a UI can show verbatim.

Returns `{ adopted: [], skipped: [] }` when the agent has no configured `working_directory`, **even if `fromWorkingDir` is given**. Placement needs the agent's own transcript folder as the destination, and per-agent session listing is scoped to that same directory, so there is nowhere to put the transcripts and nowhere they would then show up. [`listAdoptableSessions`](#listadoptablesessionsname-fromworkingdir) differs here — it falls back to `fromWorkingDir` — so it can list candidates that `adoptSessionsFrom` will then adopt none of. Give the agent a `working_directory`, or claim sessions individually with [`adoptSession`](#adoptsessionname-sessionid-opts), which moves no files and so needs none.

Placement targets the CLI transcript folder, so this is for non-Docker agents. A Docker-wrapped agent reads its sessions from `<stateDir>/docker-sessions/` and should claim them with [`adoptSession`](#adoptsessionname-sessionid-opts).
...
const plan = await manager.adoptSessionsFrom('my-agent', {
  fromWorkingDir: '/Users/ed/Code/myproject',
  dryRun: true,
});
console.log(`Would adopt ${plan.adopted.length}, skip ${plan.skipped.length}`);

// Then do it
const result = await manager.adoptSessionsFrom('my-agent', {
  fromWorkingDir: '/Users/ed/Code/myproject',
});
for (const { sessionId, reason } of result.skipped) {
  console.warn(`skipped ${sessionId}: ${reason}`);
}
```

---

### `unadoptSession(name, sessionId)`

Releases a session this agent had adopted.

```typescript
manager.unadoptSession(name: string, sessionId: string): Promise<boolean>
```
...
|-----------|------|----------|-------------|
| `name` | `string` | **Yes** | Agent qualified name or local name |
| `sessionId` | `string` | **Yes** | The session ID to release |

#### Returns

`true` if this agent's adoption record was removed.

#### Description

Removes the adoption record only — **the transcript stays on disk**. The session reverts to an ordinary unattributed native session: still visible in all-sessions views, invisible under the agent.

Returns `false` (rather than removing anything) when the session is not adopted, or when it is adopted by a *different* agent — one agent must not be able to drop another's claim.
...
Returns the resolved Claude home directory — the `claudeHomePath` constructor option, or `~/.claude` when it was omitted.

```typescript
manager.getClaudeHomePath(): string
```

This is the fleet's single source of truth for the Claude home. Embedders that need to place or inspect transcripts themselves should resolve them against this value rather than re-deriving `os.homedir()/.claude`, and callers constructing their own `RuntimeFactory`/`SDKRuntime`/`CLIRuntime` should pass it through as `claudeHomePath` so the runtime reads and appends transcripts in the same tree discovery lists from.

Unlike `getConfig()` this never returns `null` — the home is resolved in the constructor, before `initialize()`.
...
const manager = new FleetManager({
  stateDir: './.herdctl',
  claudeHomePath: '/srv/herdctl/claude-home',
});

manager.getClaudeHomePath(); // => '/srv/herdctl/claude-home'
```
