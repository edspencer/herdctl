...
<Aside type="note">
Not every job-reading method throws this error: `getJobFinalOutput()` never throws — it resolves to an empty string `""` when the job or its output can't be found.
</Aside>
...
<Aside type="note">
The class behind `INVALID_WORKING_DIRECTORY_OVERRIDE` (`InvalidWorkingDirectoryOverrideError`, thrown when a per-trigger `workingDirectory` override is not a non-empty string) is currently **not exported from the `@herdctl/core` package root** — match on `error.code` rather than `instanceof`. Tracked in [#325](https://github.com/edspencer/herdctl/issues/325).
</Aside>
