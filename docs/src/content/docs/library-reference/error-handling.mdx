...
ConfigError (base — config file parsing/loading)
├── FleetCycleError              // Circular fleet references detected
├── FleetNameCollisionError      // Duplicate sub-fleet names at same level
└── FleetLoadError               // Sub-fleet file missing or invalid
```
