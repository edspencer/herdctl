...
<Aside type="danger">
  Sessions may contain credential material in error messages (e.g., OAuth tokens during authentication failures). Exposing the web dashboard to untrusted networks creates a credential leakage risk.
</Aside>
...
1. **Keep default binding** — Use `host: localhost` unless you have a specific need and proper authentication
2. **Use reverse proxies** — Layer authentication at the proxy level (Caddy, Nginx, Traefik)
3. **Audit session files** — Review `.herdctl/web/chat-history/` for credential leaks
4. **Monitor access logs** — Watch reverse proxy logs for unauthorized access attempts
5. **Rotate credentials** — If sessions contain leaked tokens (see Finding #011 in security audits), rotate those credentials immediately
...
**See [HTTP API Authentication](/architecture/http-api/#authentication) for technical details.**
...
- [ ] Web dashboard disabled or behind authenticated reverse proxy
- [ ] Session history regularly audited for leaked credentials
