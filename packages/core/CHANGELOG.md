...
- [#431](https://github.com/edspencer/herdctl/pull/431) [`9b1e34a`](https://github.com/edspencer/herdctl/commit/9b1e34ac673f761d58f20a85ba0f6cda51d72948) Thanks [@edspencer](https://github.com/edspencer)! - Stop inferring a new CLI session's id — mint it and tell the CLI to use it (#357).
  ...
  a co-located agent _appending_ to its own session can no longer be mistaken for
...
- [#432](https://github.com/edspencer/herdctl/pull/432) [`434996a`](https://github.com/edspencer/herdctl/commit/434996aae471b1f215fd90390e67501ed8cdf178) Thanks [@edspencer](https://github.com/edspencer)! - feat(state): filter `listJobs` by a SET of agents, and page the dashboard's `/api/jobs` inside it
...
- [#438](https://github.com/edspencer/herdctl/pull/438) [`d3a704c`](https://github.com/edspencer/herdctl/commit/d3a704ce0d5e3fde8b5a3ef374a919eed805e39b) Thanks [@edspencer](https://github.com/edspencer)! - Fix session adoption offering — and silently losing — sessions owned by another agent (#437)
  ...
  job record under a _sibling_ agent resolved to
  ...
  - **One shared predicate.** The listing gate and the adoption gate encoded the
    same question in two places and drifted; that drift _was_ the bug. Both now
...
- [#434](https://github.com/edspencer/herdctl/pull/434) [`9a0d037`](https://github.com/edspencer/herdctl/commit/9a0d037bffdff80734a6e4b352c77d6d1d6f7084) Thanks [@edspencer](https://github.com/edspencer)! - feat(core): adopt pre-existing Claude Code sessions into an agent (#423)
  ...
  **New `FleetManager` methods:**
  ...
  - `unadoptSession(name, sessionId)` — release a claim; the transcript stays on
    ...
    _different_ agent, so one agent cannot drop another's claim.
  ...
  **`autoNameVersion`.** The auto-name cache is authoritative on the presence of
  ...
  _nothing_ to existing data: every already-listed session holds a current-mtime
  ...
  _field_ rather than a bump of `SessionMetadataFileSchema`'s `version` because
...
- [#433](https://github.com/edspencer/herdctl/pull/433) [`2829b0d`](https://github.com/edspencer/herdctl/commit/2829b0d55e815c29e69fd2e2885401f833218c1e) Thanks [@edspencer](https://github.com/edspencer)! - fix(core): resolve the Claude home consistently, and tell Claude Code about it (#423)
  ...
  **1. The listing path and the read path disagreed.** `SessionDiscoveryService`
  ...
  non-default home, discovery _listed_ sessions out of the configured home and then
  _read_ each one out of `~/.claude` -- where nothing was. Sessions listed but
  ...
  **2. Claude Code resolves its own home from `CLAUDE_CONFIG_DIR`, and nothing set
  ...
  them; note the SDK's `env` _replaces_ the subprocess environment rather than
...
- [#416](https://github.com/edspencer/herdctl/pull/416) [`08fbf30`](https://github.com/edspencer/herdctl/commit/08fbf304cfbe6363899a648c6372ab211659f922) Thanks [@edspencer](https://github.com/edspencer)! - perf(state): make `listJobs` skip work it throws away
...
- [#407](https://github.com/edspencer/herdctl/pull/407) [`a52b4d4`](https://github.com/edspencer/herdctl/commit/a52b4d4ac77efc5c86f6b07d323cf6bdd05e4f1f) Thanks [@edspencer](https://github.com/edspencer)! - Fix a resume self-interrupt that loses the human's turn (#406). On a real
...
- [#404](https://github.com/edspencer/herdctl/pull/404) [`89f2877`](https://github.com/edspencer/herdctl/commit/89f28771276dcd67ff7f73e63629a8c0167ef807) Thanks [@edspencer](https://github.com/edspencer)! - Fix double-resume interrupt class: `openChatSession` no longer spawns a second `claude` for a session that already has a live subprocess (edspencer/herdctl#403).
